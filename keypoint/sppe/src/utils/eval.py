...
from keypoint.sppe.src.utils.img import transformBoxInvert, transformBoxInvert_batch
import torch
...
def get_prediction(hms, pt1, pt2):
    """
    Get keypoint location from heatmaps

    Args
    - hms: (Tensor) Heatmaps with size [BS x KPNUM x W x H]

    Returns
    - preds: (Tensor) Keypoints prediction with size [BS x KPNUM x 2]
    - maxval: (Tensor) Keypoints prediction score with size [BS x KPNUM]
    """
    ...
    preds_tf = transformBoxInvert_batch(preds, pt1, pt2, 320, 256, 80, 64)
